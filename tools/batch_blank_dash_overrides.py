#!/usr/bin/env python3
"""
Batch-apply manual audio overrides for every texts.json entry containing the
"[[blank:item-N]]" fill-in-the-blank template token, so read-aloud says
"dash" for each blank instead of literally reading "blank item 9" (item 5 /
item 7 in the evaluation comments). Visible text/HTML (which the runtime
parses to render the actual input widget) is left untouched.
...
BLANK_RE = re.compile(r"\[\[blank:[a-zA-Z0-9_-]+\]\]")
...
def main():
    ...
    ids = sorted(k for k, v in texts.items() if isinstance(v, str) and "[[blank:" in v)
    todo = [k for k in ids if k not in manual_ids]
    print(f"Total blank-template ids: {len(ids)}; already manual: {len(ids) - len(todo)}; to do: {len(todo)}", flush=True)
    ...
    for i, text_id in enumerate(todo, start=1):
        ...
        spoken = BLANK_RE.sub("dash", spoken_base)
        ...
        while True:
            ...
            try:
                ...
                print(f"OK [{i}/{len(todo)}] {text_id}: {spoken[:90]!r}", flush=True)
                break
